...
export default async ({ client, command, respond }, args) => {
  ...
  if (
    !(await isUserInChannel(client, command.user_id, config.reviewChannelId))
  ) {
  ...
  if (!post.lockedDownAt) {
    await respond('This post is already unlocked.')
    return
  }
  ...
  const parentId = await getParentMessageId(
    client,
  ...
  await sendMessage(client, config.streamChannelId, {
    text: `_${command.user_name}(${command.user_id}) unlocked <${postPermalink}|*#${post.postNumber}*>._`,
    unfurl_links: false
  })
}

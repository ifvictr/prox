...
export default app => {
  app.event(
    ...
    async ({ client, event }) => {
      await sendMessage(
        ...
        `_Someone joined <#${event.channel}>._`
      )
    }
  )
  ...
  app.message(
    ...
    async ({ client, context, message }) => {
      ...
      if (
        !(
          deletedMessage.user === context.botUserId ||
          deletedMessage.bot_id === context.botId
        )
      ) {
        return
      }
      ...
      let messageType
      if (deletedMessage.ts === post.postMessageId) {
        // Was a post
        messageType = 'post'
        await post.delete() // Sync post deletion to database
      } else if (
        deletedMessage.thread_ts === post.postMessageId &&
        deletedMessage.username
      ) {
        // Was a reply
        messageType = 'reply'
      } else {
        // We don't care about notification messages
        return
      }

      const displayName = deletedMessage.user
        ? `<@${deletedMessage.user}>`
        : deletedMessage.username
      ...
      await sendMessage(client, config.streamChannelId, {
        text: `_${
          messageType !== 'post'
            ? `A ${messageType} from ${displayName} under `
            : ''
        }${postNumberText} was deleted:_\n>>> ${removeSpecialTags(
          deletedMessage.text
        )}`,
        unfurl_links: false
      })
